...
public class MinerExecution_OLD1512 {
	public static void main(String[] args) {
		...
		BigDecimal valorPote = new BigDecimal(150);
			...
			try {
				...
				for (int indexNamesPlayers = 0; indexNamesPlayers < (size>6?6:size); indexNamesPlayers++) {
					...
					for (int indexGameAtual = 0; indexGameAtual < gameActual.size(); indexGameAtual++) {
						...
						int numeroPartidafinalizada  = 1;// se n�o houver, poss�vel estar no primeiro, validar regra
						...
						try {
							...
							if (!mapGames.containsKey(keyEntrySet) || ( mapGames.get(keyEntrySet) == null || mapGames.get(keyEntrySet).getNumeroPartidaFinalizada() != numeroPartidafinalizada)) {
							...
							if(mapGames.get(keyEntrySet).getJogador().getPontos().equals("40") 
							   ...
							   (mapGames.get(keyEntrySet).getJogador().getPontos().equals("A") 
								...
								if(mapGamesPerdidos.get(keyEntrySet)!=null
										&& mapGamesPerdidos.get(keyEntrySet).getQuantidadeEmpates()>0){
									numeroEmpates = numeroEmpates+1;
								}
								mapGames.get(keyEntrySet).setQuantidadeEmpates(numeroEmpates);
								mapGames.get(keyEntrySet).setPartidaEmpatada(true);

								//quando cirar uma nova, quando zerar os pontos e mudar o jogo, n�o pode estar block
								...
								//valorPote = valorPote.add(new BigDecimal(-3));
								System.out.println("Valor Atual: "+valorPote);
								System.out.println("Horario: "+mapGames.get(keyEntrySet).getHorarioPartida());
								System.out.println("\n");
								//mapGamesPerdidos.put(keyEntrySet+"_numeropartida"+numeroPartidaAtual, mapGames.get(keyEntrySet));
								mapGamesPerdidos.put(keyEntrySet, mapGames.get(keyEntrySet));
								
								mapGames.remove(keyEntrySet);
								continue;
								
							//}else if(mapGames.get(keyEntrySet).getNumeroPartidaAnterior() < numeroPartida) {
							...
							if(mapGamesPerdidos.get(keyEntrySet)!=null && mapGamesPerdidos.get(keyEntrySet)!=null
							...
							}/*else if(mapGamesPerdidos.get(keyEntrySet)!=null
									&& !mapGamesPerdidos.get(keyEntrySet).isPartidaGanha()
									&& mapGamesPerdidos.get(keyEntrySet).getQuantidadeEmpates() < Miner.MAXIMO_EMPATES){
								mapGames.get(keyEntrySet).setPartidaAnterior(mapGamesPerdidos.get(keyEntrySet));
							}
							*/
							...
							for (int i = 0; i < listsApostasIndividuais.size(); i++) {
								...
								System.out.println("Jogadores: "+mapGames.get(keyEntrySet).getJogador().getNome()+" VS "+mapGames.get(keyEntrySet).getJogadorAdversario().getNome());
								System.out.println("Entrou na Partida "+mapGames.get(keyEntrySet).getJogador().getPontos()+" - "+mapGames.get(keyEntrySet).getJogadorAdversario().getPontos());
