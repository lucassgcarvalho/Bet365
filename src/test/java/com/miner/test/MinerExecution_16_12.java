...
public class MinerExecution_16_12 {
	public static void main(String[] args) {
		...
		while (true) {
			try {
				...
				for (int indexNamesPlayers = 0; indexNamesPlayers < (size > 6 ? 6 : size); indexNamesPlayers++) {
					...
					List<WebElement> gameActual = abaPartida.findElements(
							By.xpath(String.format(Bot.XPATH_GAME_BY_PLAYER, player.getText().toString())));
					for (int indexGameAtual = 0; indexGameAtual < gameActual.size(); indexGameAtual++) {
						...
						String[] pontos = elementGameAtual.findElement(By.xpath(Bot.XPATH_PONTOS_PLAYERS)).getText()
								.split("\n");
						...
						try {
							...
							// \\\"]")));
							gameEmExecucao = elementGameAtual
									.findElement(By.xpath("//div[@class=\"ipe-SummaryButton_Label \"]"));
							stringNumeroJogoAtual = gameEmExecucao.getText().split("-")[0].split("Jogo")[1];
							numeroPartidafinalizada = Integer.parseInt(loco.getOnlyNumbers(stringNumeroJogoAtual));
						} catch (Exception e) {
							//e.printStackTrace();
							// System.out.println("Erro - In�cio de jogo:"+playersNamesActual[0]+" VS
						...
						try {
							try {
								if(!loco.isPresent(By.xpath(Bot.XPATH_LINK_ABA_DEUCE))) {
									throw new Exception("Aba Deuce n�o est� vis�vel - "+playersNamesActual[0] +" VS "+playersNamesActual[1]);
								}
							...
							String keyEntrySet = Bot.getEntrySetName(nomeCampeonato, playersNamesActual[0],
									playersNamesActual[1]);
							// cria nova estrutura
							if (!mapGames.containsKey(keyEntrySet) || (mapGames.get(keyEntrySet) == null
									|| mapGames.get(keyEntrySet).getNumeroPartidaFinalizada() != numeroPartidafinalizada)) {
								Partida partida = new Partida();
								partida.setCampeonato(nomeCampeonato);
								partida.setNumeroPartidaAnterior(
										new Integer(mapGames.get(keyEntrySet) == null ? numeroPartidaAtual
												: mapGames.get(keyEntrySet).getNumeroPartida()));
								partida.setNumeroPartida(numeroPartidaAtual);
								partida.setNumeroPartidaFinalizada(numeroPartidafinalizada);
								...
								partida.setPartidaAnterior(
										mapGames.get(keyEntrySet) == null ? null : mapGames.get(keyEntrySet));
								// partida.setQuantidadeEmpates(mapGames.get(keyEntrySet) == null ? 0:
								// mapGames.get(keyEntrySet).getQuantidadeEmpates());//add 14/12 20:41
						...
						if ((mapGames.get(keyEntrySet).getJogador().getPontos().equals("40")
								...
								&& mapGames.get(keyEntrySet).isPartidaBloqueada()
								&& !mapGames.get(keyEntrySet).isPartidaEmpatadaProcessada()
								&& mapGames.get(keyEntrySet).getNumeroPartidaFinalizada() == numeroPartidafinalizada){
								...
								// int numeroEmpates =
								// mapGames.get(keyEntrySet).getQuantidadeEmpates()==null?0:mapGames.get(keyEntrySet).getQuantidadeEmpates();

								
								if(mapGamesPerdidos.get(keyEntrySet)!=null) {
									mapGames.get(keyEntrySet).setQuantidadeEmpates(
											mapGamesPerdidos.get(keyEntrySet).getQuantidadeEmpates()==null?
													1:mapGamesPerdidos.get(keyEntrySet).getQuantidadeEmpates() + 1);
								}else {
									mapGames.get(keyEntrySet).setQuantidadeEmpates(
											mapGames.get(keyEntrySet).getQuantidadeEmpates()==null?
													1:mapGames.get(keyEntrySet).getQuantidadeEmpates() + 1);
								}
								...
								System.out.println("Numero Partida: " + numeroPartidaAtual);
								...
								System.out.println(
										"N�mero empates seguidos: " + mapGames.get(keyEntrySet).getQuantidadeEmpates());
								System.out.println("Valor Aposta: " + mapGames.get(keyEntrySet).getValorAposta());
								System.out.println(
										"Retorno Aposta: " + mapGames.get(keyEntrySet).getValorRetornoAposta());
								...
								// < numeroPartida) {
									...
									&& mapGames.get(keyEntrySet).getPartidaAnterior()
											.getNumeroPartidaFinalizada() < numeroPartidafinalizada
									&& mapGames.get(keyEntrySet).getPartidaAnterior().isPartidaBloqueada()
							/* && mapGames.get(keyEntrySet).isPartidaBloqueada() */) {
								// System.out.println("Trocando de partida de
								// "+mapGamesGanhos.get(keyEntrySet).getNumeroPartidaAnterior()+" para
								// "+mapGames.get(keyEntrySet).getNumeroPartida());
								BigDecimal valorPoteAnterior = valorPote;
								valorPote = valorPote
										.add(mapGames.get(keyEntrySet).getPartidaAnterior().getValorRetornoAposta());

								System.out.println("Aposta ganha");
								System.out.println("Trocando de partida de "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getNumeroPartidaFinalizada()
										+ " para " + numeroPartidafinalizada);
								System.out.println("Numero Partida ganha: "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getNumeroPartidaFinalizada());
								System.out.println("Numero proxima Partida " + numeroPartidafinalizada);
								System.out.println("Jogadores: "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getJogador().getNome() + " VS "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getJogadorAdversario()
												.getNome());
								System.out.println("Valor Aposta: "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getValorAposta());
								System.out.println("Retorno Aposta: "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getValorRetornoAposta());
								System.out.println("Valor anterior: " + valorPoteAnterior);
								System.out.println("Valor Atual: " + valorPote);
								System.out.println("Horario: "
										+ mapGames.get(keyEntrySet).getPartidaAnterior().getHorarioPartida());
								System.out.println("\n");
								...
								mapGamesGanhos.put(keyEntrySet + "_numeropartida" + numeroPartidaAtual,
										mapGames.get(keyEntrySet).getPartidaAnterior());
								...
								// se apenas for 40-40n n�o fazer jogada
									...
									&& mapGames.get(keyEntrySet).getJogadorAdversario().getPontos().equals("40")
									|| (mapGames.get(keyEntrySet).getJogador().getPontos().equals("A") || mapGames
											.get(keyEntrySet).getJogadorAdversario().getPontos().equals("A"))) {
								continue;
							}
							...
							if (mapGamesPerdidos.get(keyEntrySet) != null 
								...
								System.out.println("Partida Bloqueada por excesso de emaptes: "+mapGames.get(keyEntrySet).getQuantidadeEmpates());
								...
								System.out.println("N�mero Partida: "+mapGames.get(keyEntrySet).getNumeroPartida());
								mapGamesPerdidos.remove(keyEntrySet);
								mapGames.remove(keyEntrySet);
								continue;
							}
							if (mapGames.get(keyEntrySet) != null && mapGames.get(keyEntrySet).isPartidaBloqueada()) {
								continue;
							}
							...
							loco.waitForCondition(
									ExpectedConditions.visibilityOfElementLocated(By.xpath(Bot.XPATH_SPAN_ODDS)));
							WebElement webElementValorOdds = deuce
									.findElements(By.xpath("../.." + Bot.XPATH_SPAN_ODDS)).get(1);
							...
							List<WebElement> listTodasApostas = switchToFrame
									.getWebElements(By.xpath(Bot.XPATH_LI_TODAS_APOSTAS));

							List<WebElement> listsApostasIndividuais = listTodasApostas.get(0)
									.findElements(By.xpath(Bot.XPATH_LI));
							...
							for (int i = 0; i < listsApostasIndividuais.size(); i++) {
								...
								if (mapGamesPerdidos.get(keyEntrySet) != null
										...
										&& mapGamesPerdidos.get(keyEntrySet).getQuantidadeEmpates() <= Bot.MAXIMO_EMPATES) {
									valorAposta = bot.calcularValorAposta(mapGamesPerdidos.get(keyEntrySet).getValorAposta(),true, mapGamesPerdidos.get(keyEntrySet)).add(new BigDecimal(Bot.VALOR_PADRAO_APOSTA));
								} else
									valorAposta = bot.calcularValorAposta(Bot.VALOR_PADRAO_APOSTA, false, mapGamesPerdidos.get(keyEntrySet));
								...
								if (producao) {
									...
									while (!apostaEfetuada) {
										try {
											...
											if(loco.isPresent(By.xpath("//div[text()='Aceitar Altera��es']"))){
												loco.getWebElement(By.xpath("//div[text()='Aceitar Altera��es']")).click();
											...
											// bs-BtnWrapper']"));
											/*WebElement btnAceitarAlteracaoAposta = loco.getWebElement(By.xpath("//a[@class='acceptChanges bs-BtnWrapper']"));
								...
								System.out.println("Jogadores: " + mapGames.get(keyEntrySet).getJogador().getNome()
										+ " VS " + mapGames.get(keyEntrySet).getJogadorAdversario().getNome());
								System.out.println(
										"Entrou na Partida " + mapGames.get(keyEntrySet).getJogador().getPontos()
								...
								System.out.println(
										"Valor Anterior: " + valorPote.add(new BigDecimal(Bot.VALOR_PADRAO_APOSTA)));
								System.out.println("Valor Atual: " + valorPote);
								System.out.println("Horario: " + horarioAposta.getText());
								System.out.println("\n");
							}
						} catch (Exception e) {
							//e.printStackTrace();
