...
public class CopyOfBotExecution0401 {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		...
		BigDecimal valorPote = new BigDecimal(900);
		...
		while (true) {
			try {
				...
				//for (int indexNamesPlayers = 0; indexNamesPlayers <  size; indexNamesPlayers++) {
						...
						try {
							...
							if (!mapGames.containsKey(keyEntrySet)
								...
								partida.setNumeroPartidaAnterior(
										new Integer(mapGames.get(keyEntrySet) == null ? numeroApostaAtual: mapGames.get(keyEntrySet).getNumeroPartida()));
								...
								//partida.setPartidaAnterior(mapGames.get(keyEntrySet) == null ? null: mapGames.get(keyEntrySet));// talvez estivesse fazendo com que fosses processadas duas vezes as perdidas
							...
							if (((partidaAtual.getJogador().getPontos().equals("40")
									...
									//TODO: melhorar a logica para pegar o n�mero da partida que foi apostada e nao somente a que est� no 40 - 40
							...
							} else if (bot.isGame40_A_A_40(partidaAtual)) {
								continue;
							}
							...
							for (int i = 0; i < listsApostasIndividuais.size(); i++) {
								...
								partidaAtual.setValorRetornoAposta(bot.convertStringToBigDecimal(valor.trim()));
								//partidaAtual.setNumeroPartida(numeroApostaAtual);
								
								WebElement horarioAposta = null;
								
								if (bot.executarAposta(mapGames, loco, keyEntrySet, Bot.PRODUCAO, apostaSelecionada)) {

									partidaAtual.setPartidaBloqueada(true);
									loco.driver.switchTo().defaultContent();
									horarioAposta = loco.getWebElement(By.xpath(Bot.XPATH_TIME_SERVER));
									valorPote = valorPote.subtract(valorAposta).setScale(2, RoundingMode.HALF_UP);
